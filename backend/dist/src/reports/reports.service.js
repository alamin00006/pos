...
let ReportsService = class ReportsService {
    ...
    async getTodayReport() {
        ...
        const [sales, purchases, expenses, returns] = await Promise.all([
            this.prisma.sale.aggregate({
                where: { saleDate: { gte: startDate, lt: endDate }, status: client_1.SaleStatus.COMPLETED, deletedAt: null },
                _sum: { total: true, paidAmount: true, dueAmount: true },
                _count: true,
            }),
            this.prisma.purchase.aggregate({
                where: { purchaseDate: { gte: startDate, lt: endDate }, status: client_1.PurchaseStatus.RECEIVED, deletedAt: null },
    ...
    async getDailyReport(query) {
        ...
        const purchases = await this.prisma.purchase.findMany({
            where: { ...where, purchaseDate: where.saleDate, status: client_1.PurchaseStatus.RECEIVED },
    ...
    async getCurrentMonthReport() {
        ...
        const [sales, purchases, expenses, returns, damages] = await Promise.all([
            this.prisma.sale.aggregate({
                where: { saleDate: { gte: startDate, lte: endDate }, status: client_1.SaleStatus.COMPLETED, deletedAt: null },
                _sum: { total: true, paidAmount: true, dueAmount: true, discount: true },
                _count: true,
            }),
            this.prisma.purchase.aggregate({
                where: { purchaseDate: { gte: startDate, lte: endDate }, status: client_1.PurchaseStatus.RECEIVED, deletedAt: null },
                _sum: { total: true, paidAmount: true, dueAmount: true },
                _count: true,
            }),
            this.prisma.expense.aggregate({
    ...
    async getProfitLossReport(query) {
        const { startDate, endDate } = this.getDateRange(query);
        const dateFilter = {};
        if (startDate)
    ...
    async getTopProductsReport(query) {
        const { startDate, endDate } = this.getDateRange(query);
    ...
    async getTopCustomersReport(query) {
        const { startDate, endDate } = this.getDateRange(query);
        const limit = query.limit || 10;
        const dateFilter = {};
        if (startDate)
    ...
    async getCategoryWiseReport(query) {
        const { startDate, endDate } = this.getDateRange(query);
        const dateFilter = {};
        if (startDate)
            dateFilter.gte = startDate;
    ...
    async getCustomerDueReport(query) {
        const where = { deletedAt: null };
        if (query.customerId)
        ...
        const customers = await this.prisma.customer.findMany({
            ...
            include: {
                sales: {
                    where: { status: client_1.SaleStatus.COMPLETED, deletedAt: null, dueAmount: { gt: 0 } },
    ...
    async getSupplierDueReport(query) {
        const where = { deletedAt: null };
        if (query.supplierId)
        ...
        const suppliers = await this.prisma.supplier.findMany({
            ...
            include: {
                purchases: {
                    where: { status: client_1.PurchaseStatus.RECEIVED, deletedAt: null, dueAmount: { gt: 0 } },
    ...
    async getSummaryReport() {
        const [totalCustomers, totalSuppliers, totalProducts, totalSales, totalPurchases, totalExpenses, totalReturns,] = await Promise.all([
            this.prisma.customer.count({ where: { deletedAt: null } }),
            this.prisma.supplier.count({ where: { deletedAt: null } }),
            this.prisma.product.count({ where: { deletedAt: null } }),
            this.prisma.sale.aggregate({
                where: { status: client_1.SaleStatus.COMPLETED, deletedAt: null },
                _sum: { total: true, paidAmount: true, dueAmount: true },
                _count: true,
            }),
            this.prisma.purchase.aggregate({
                where: { status: client_1.PurchaseStatus.RECEIVED, deletedAt: null },
                _sum: { total: true, paidAmount: true, dueAmount: true },
                _count: true,
            }),
            this.prisma.expense.aggregate({
