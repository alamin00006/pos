...
let SalesService = class SalesService {
    ...
    async findAll(query) {
        ...
        const where = { ...this.prisma.notDeleted() };
        if (invoiceNo)
        ...
        const [sales, total] = await Promise.all([
            this.prisma.sale.findMany({
                ...
                skip,
                take,
                orderBy: (0, pagination_util_1.buildOrderByQuery)(sortBy || 'saleDate', sortOrder || 'desc'),
                include: {
        ...
        return (0, pagination_util_1.paginate)(salesWithProfit, total, page, limit);
    }
    async findOne(id) {
        ...
        return this.prisma.$transaction(async (tx) => {
                ...
                for (const payment of dto.payments) {
                    await tx.payment.create({
                        data: {
                            ...
                            paymentMethod: payment.paymentMethod || client_1.PaymentMethod.CASH,
                            note: payment.note
                        }
                    });
                    if ((payment.paymentMethod || client_1.PaymentMethod.CASH) === client_1.PaymentMethod.CASH) {
                        const lastCashEntry = await tx.cashBook.findFirst({ orderBy: { createdAt: 'desc' } });
                        await tx.cashBook.create({
                            data: {
                                type: client_1.CashBookType.IN,
                                source: client_1.CashBookSource.SALE,
                                referenceId: sale.id,
                                amount: payment.amount,
            ...
            for (const item of itemsWithTotal) {
                await tx.stockLedger.create({
                    data: {
                        ...
                        source: client_1.StockLedgerSource.SALE,
                        referenceId: sale.id,
                        quantity: item.quantity,
            ...
            if (dto.customerId && dueAmount > 0) {
                ...
                await tx.customerLedger.create({
                    data: {
                        customerId: dto.customerId,
                        type: client_1.CustomerLedgerType.SALE_DUE,
                        referenceId: sale.id,
                        amount: dueAmount,
    ...
    async addPayment(saleId, dto) {
        return this.prisma.$transaction(async (tx) => {
            ...
            if ((dto.paymentMethod || client_1.PaymentMethod.CASH) === client_1.PaymentMethod.CASH) {
                const lastCashEntry = await tx.cashBook.findFirst({ orderBy: { createdAt: 'desc' } });
                await tx.cashBook.create({
                    data: {
                        type: client_1.CashBookType.IN,
        ...
        return this.prisma.$transaction(async (tx) => {
            ...
            if (sale.customerId) {
                ...
                await tx.customerLedger.create({
                    data: {
                        customerId: sale.customerId,
                        type: client_1.CustomerLedgerType.RETURN_ADJUST,
                        referenceId: saleReturn.id,
                        amount: returnTotal,
            ...
            if (Number(totalRefunded._sum.total) >= Number(sale.total)) {
                await tx.sale.update({
                    where: { id: saleId },
                    data: { status: client_1.SaleStatus.REFUNDED }
                });
            }
            return saleReturn;
        });
    ...
    async getCustomerDue(tx, customerId) {
